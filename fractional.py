...
def to_formula(X_train, components=None, compositions=None):
    """Convert training DataFrame of compositions into formulas (strings).
    ...
    X_train : DataFrame
        Fractionally encoded matrix (similar to a one-hot encoded matrix).
    components : list of list of str
        Can be ragged. The components that correspond to a given compound. If None, then
        use `X_train`. By default
        None.
    compositions : list of list of str
        Can be ragged. Must be same "ragged shape" as components. The compositions that
        correspond to the components in a given compound. If None then use `X_train`. By default None.
    ...
    ------
    ValueError
        Either X_train or (components and compositions) should be specified
    """
    err_msg = "Either X_train or (components and compositions) should be specified"
    if X_train is not None:
        if components is not None or compositions is not None:
            raise ValueError(err_msg)
        components, compositions = fractional_decode(X_train)
    else:
        if components is None or compositions is None:
            raise ValueError(err_msg)
    formulas = []
    for component, composition in zip(components, compositions):
        formula = [
            element + f"{fraction:.9f}"
            for element, fraction in zip(component, composition)
        ]
